...
for rep = 1:100
    ...
    for i = 1:length(k)
        ...
        mu1 = logistic(X*β)
        socp = sagnol_A(Diagonal(w)*X[rest,:], μ*Diagonal(ones(p)), k[i]; K=(Diagonal(mu1 .* (1 .- mu1))*X)', verbose=0, IC=0)
        cand = rest[rand(n-k[i]) .< socp]
        append!(cand, pre)
        β = irls(X[cand,:],y[cand],μ=μ)
        ...
        @save "delta-Copy1.jld2" acc_train auc_train acc_test auc_test
