...
for rep = 1:100
    ...
    for i = 1:length(k)
        ...
        # OED, MISOCP
        socp = sagnol_A(Diagonal(w)*X[rest,:], μ*Diagonal(ones(p)), k[i]; verbose=0, IC=1)
        cand = rest[BitArray(round.(socp))]
        append!(cand, pre)
        ...
        mu1 = logistic(X*β)
        socp = sagnol_A(Diagonal(w)*X[rest,:], μ*Diagonal(ones(p)), k[i]; K=(Diagonal(mu1 .* (1 .- mu1))*X)', verbose=0, IC=1)
        cand = rest[BitArray(round.(socp))]
        append!(cand, pre)
        β = irls(X[cand,:],y[cand],μ=μ)
        ...
        @save "delta2.jld2" acc_train auc_train acc_test auc_test
